 ...
 * 位置误差每周期更新并参与 LQR；速度目标归零时锁存停车位置。
 * 每轮最终限流±1.5 A。
 */
typedef struct
{
    ...
    /* 转向 PID 使用左右轮物理速度差换算的角速度作为反馈。 */
    Class_PID Turn_PID;  /* 转向角速度误差 -> 左右轮差动电流，A。 */

    /* 外部目标值。 */
    float Target_Angle;      /* 已标定直立零点，rad；LQR 不覆盖。 */
    float Target_Speed;      /* 目标前进速度，单位 rpm；正负表示前进或后退。 */
    float Target_Turn_Angle; /* 目标转向角速度，单位 rad/s；正负表示左右转。 */

    float Speed_Feedback;    /* 原始合成轮速rpm；LQR使用修正后的Speed_M_S。 */
    /* 四状态增益G=-K；倾角、角速度、速度误差和位置误差均参与输出。 */
    ...
    float Position_Target;   /* 停车点目标，m；非零速度目标时跟随当前位置，目标刚归零时锁存一次。 */
    float Position_Error;    /* Position - Position_Target，m；每周期更新，供 LQR 位置项使用。 */
 ...
 * lqr_pos调整位置误差到共同电流的增益，单位A/m；当前位置反馈已启用。
 * turn_kp/turn_ki/turn_kd:数值# 调整转向PID。旧 angle_*、speed_* 增益键已移除，
 * 避免将旧PID单位的参数误写入LQR。在线改增益不清位置，也不改标定零点。
 * speed:0# 设置零速目标；从非零目标切回零时锁存当前位置，后续位置误差参与LQR。
 * 已为零速目标时重复发送不会重新锁存停车点；上电自动启动不变。
